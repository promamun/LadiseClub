  ...
  if (dt_projects_table.length) {
    var dt_project = dt_projects_table.DataTable({
      ...
      columns: [
        ...
        { data: '',
          render: function (data, type, full, meta) {
            var editUrl = assetsPath + 'admin/slider/edit/' + full.id; // Assuming full.id contains the member's ID
            var deleteUrl = assetsPath + 'admin/slider/delete/' + full.id; // Assuming full.id contains the member's ID
            return (
      ...
      buttons: [
        {
          text: '<i class="ti ti-plus me-sm-1"></i><span class="d-none d-sm-inline-block">Add New Slider</span>',
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of "' + data['name'] + '" Slider';
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Sliders</h5>');
  }
  // Add event listener to the button to navigate to the URL
  $('.create-new').on('click', function() {
    var addUrl = assetsPath+'admin/slider/add';
