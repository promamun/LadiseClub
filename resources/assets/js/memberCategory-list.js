...
(function () {
  //  For Datatable
  // --------------------------------------------------------------------
  var dt_projects_table = $('.datatables-member-category');
  ...
  if (dt_projects_table.length) {
    var dt_project = dt_projects_table.DataTable({
      ...
      columns: [
        ...
        { data: '',
          render: function (data, type, full, meta) {
            ...
            var deleteUrl = assetsPath + 'admin/member-category/delete/' + full.id; // Assuming full.id contains the member's ID
            return (
      ...
      buttons: [
        {
          text: '<i class="ti ti-plus me-sm-1"></i><span class="d-none d-sm-inline-block">Add New Member Category</span>',
      ...
      responsive: {
        details: {
          display: $.fn.dataTable.Responsive.display.modal({
            header: function (row) {
              var data = row.data();
              return 'Details of "' + data['name'] + '" Members';
            }
          }),
          type: 'column',
    ...
    $('div.head-label').html('<h5 class="card-title mb-0">Member Categories</h5>');
  }
  // Add event listener to the button to navigate to the URL
  $('.create-new').on('click', function() {
    var addUrl = assetsPath+'admin/member-category/add';
